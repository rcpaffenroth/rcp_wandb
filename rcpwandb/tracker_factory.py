...
class TrackerFacade:
    def __init__(self, tracker, tracker_type, console_level='info'):
        self._tracker = tracker
        self._tracker_type = tracker_type

        self.console_logger = logging.getLogger('TrackerFacade')
        # # This bears some explanation.  We want to be able to control the
        # # the logging level even when the tracker is running in a separate
        # # process.  For example, this happens when the tracker is run
        # # through Dask.
        ...
        self.console_logger.info(f"tracker_type {self._tracker_type}")
    ...
    def log_image(self, name, image, step=None):
        if self._tracker_type == 'wandb':
            self._tracker.log({name: wandb.Image(image)}, step=step)
    ...
    def log_plot(self, name, fig, step=None):
        if self._tracker_type == 'wandb':
            self._tracker.log({name: wandb.Plotly(fig)}, step=step)
        self.console_logger.debug(f"name {name} step {step}")

    def log_video(self, name, path_to_video, step=None):
        if self._tracker_type == 'wandb':
            self._tracker.log({name: wandb.Video(path_to_video)}, step=step)
        self.console_logger.debug(f"name {name} step {step}")
    ...
    def log_dataframe(self, name, df, step=None):
        if self._tracker_type == 'wandb':
            self._tracker.log({name: df}, step=step)
        else:
            self.console_logger.warning(f"log_dataframe not implemented for tracker type {self._tracker_type}")  
    ...
    def save_model(self, model_path, model_name):
        if self._tracker_type == 'wandb':
            ...
            self.console_logger.warning(f"save_model not implemented for tracker type {self._tracker_type}") 
...
def TrackerFactory(type='console', console_level='info',
    ...
    return TrackerFactoryFromConfig(cfg, model, default_root_dir)

def TrackerFactoryFromConfig(cfg=None, model=None, default_root_dir=None):
    global tracker
    if tracker is not None:
        tracker.console_logger.warning("Tracker already created, recreating")
    ...
    if cfg['type'] == 'wandb':
        ...
        tracker = TrackerFacade(wandb, 'wandb', cfg['console_level'])
        ...
        tracker = TrackerFacade(None, 'console', cfg['console_level'])
    else:
        raise ValueError(f"Unknown tracker type: {cfg['type']}")
